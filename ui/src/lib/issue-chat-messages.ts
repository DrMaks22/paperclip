...
function authorNameForComment(
  ...
  if (!authorUserId) return options?.isSystemNotice ? "Paperclip" : t("onboarding.chat.you");
  const userLabel = userLabelMap?.get(authorUserId)?.trim();
  if (userLabel) return userLabel;
  return formatAssigneeUserLabel(authorUserId, currentUserId, userLabelMap) ?? t("onboarding.chat.you");
}
...
function createTimelineEventMessage(args: {
  ...
  const actorName = event.actorType === "agent"
    ...
    : event.actorType === "system"
      ...
      : (formatAssigneeUserLabel(event.actorId, currentUserId, userLabelMap) ?? t("pages.cliAuth.board"));
  ...
  if (event.assigneeChange) {
    const from = event.assigneeChange.from.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.from.userId, currentUserId, userLabelMap) ?? t("filter.unassigned"));
    const to = event.assigneeChange.to.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.to.userId, currentUserId, userLabelMap) ?? t("filter.unassigned"));
...
/** Display-only projection of labels generated by this module. Keep raw message
 * metadata for transcript classifiers, copy/export, and history identity. */
export function issueChatRunLabelDisplay(value: string, durationDisplay: (duration: string) => string): string {
  ...
  const match = /^(Worked for|Failed after|Timed out after|Interrupted by board after|Paused by board after|Cancelled after) (.+)$/.exec(value);
  if (!match) return value;
  const keys: Record<string, string> = { "Worked for": "runWorked", "Failed after": "runFailed", "Timed out after": "runTimedOut", "Interrupted by board after": "runInterrupted", "Paused by board after": "runPaused", "Cancelled after": "runCancelled" };
  return t(`localizationTaskRuntime.${keys[match[1]]}`, { duration: durationDisplay(match[2]) });
}
...
function runDurationLabel(run: {
  ...
  switch (run.status) {
    case "succeeded":
      return durationText ? `Worked for ${durationText}` : t("localizationTaskRuntime.display.ui_Finished_work_131omes");
    case "failed":
    case "error":
      return durationText ? `Failed after ${durationText}` : t("localizationTaskRuntime.display.ui_Run_failed_1qwj1un");
    case "timed_out":
      return durationText ? `Timed out after ${durationText}` : t("localizationTaskRuntime.display.ui_Run_timed_out_1chh2yl");
    case "cancelled":
      if (isOperatorInterruptedRun(run.resultJson, run.errorCode)) {
        return durationText ? `Interrupted by board after ${durationText}` : t("localizationTaskRuntime.display.ui_Interrupted_by_board_1tvp8be");
      }
      if (stopReason === "paused") {
        return durationText ? `Paused by board after ${durationText}` : t("localizationTaskRuntime.display.ui_Paused_by_board_1r4kx68");
      }
      return durationText ? `Cancelled after ${durationText}` : t("localizationTaskRuntime.display.ui_Run_cancelled_137hdc9");
    case "queued":
      return t("status.queued");
    case "running":
      return t("localizationTaskRuntime.display.ui_Working_tx9t2m");
    default:
...
function createHistoricalTranscriptMessage(args: {
  ...
  const waitingText = hasOutput ? "" : t("localizationTaskRuntime.display.ui_Run_finished_6lpgzk");
...
function createLiveRunMessage(args: {
  ...
  const waitingText =
    run.status === "queued"
      ? t("localizationTaskRuntime.display.ui_Queued_u04tey")
      : parts.length > 0
        ? ""
        : t("localizationTaskRuntime.display.ui_Working_tx9t2m");

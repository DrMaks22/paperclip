...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({ selectedCompany: { id: "company-raw", issuePrefix: "RAW" } }),
}));
...
describe("final agent chrome localization", () => {
  ...
  it.each([
    ...
    expect(status.querySelector("p")?.textContent).toBe(`${count} governed tool ${count === 1 ? "call was" : "calls were"} denied or failed in the last hour. Access is enforced server-side by the tool gateway — review what was blocked and why in the audit log.`);
    const auditLink = status.querySelector("a")!;
    expect(auditLink.textContent).toBe("View audit →");
    expect(auditLink.getAttribute("href")).toBe("/RAW/apps/advanced/audit");
    ...
    expect(status.querySelector("a")).toBe(auditLink);
    expect(auditLink.textContent).toBe("Открыть журнал аудита →");
    expect(auditLink.getAttribute("href")).toBe("/RAW/apps/advanced/audit");

...
const codexThinkingEffortOptions = [
  ...
  { id: "minimal", get "label"() { return t("localizationIssueLists.effortMinimal"); } },
  ...
  { id: "xhigh", get "label"() { return t("localizationIssueLists.effortExtraHigh"); } },
] as const;
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      ...
      const testResults: Array<{ label: string; model: string | null; result: AdapterEnvironmentTestResult }> = [
        {
          get "label"() { return t("localizationIssueDetail.ui_Primary_model"); },
          model: primaryModel,
      ...
      if (cheapTestCase) {
        testResults.push({
          get "label"() { return t("stableCore.cheapModel"); },
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("localizationAgents.ui30_Identity")}</h3>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label={t("pages.inviteLanding.agentForm.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("localizationAgents.ui120_Describe_what_this_agent_can_do_")}
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("localizationAgents.ui38_Adapter")}</h3>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("localizationAgents.ui136_Permissions_Configuration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("localizationAgents.ui136_Permissions_Configuration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              {/*
              ...
              */}
              ...
              {supportsModelProfiles && (
                <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("localizationIssueDetail.ui_Primary_model")}</div>
              )}
              <ModelDropdown
                models={models}
              ...
              <Field label={t("localizationAgents.ui146_Environment_variables")} hint={help.envVars}>
                <EnvironmentVariablesEditor
...
function DisplayedCodeLoginPanel({
  ...
  const startLogin = useMutation({
    ...
    onError: (error) => {
      setStartError(error instanceof Error ? error.message : t("localizationAgents.config_Could_not_start_the_login_"));
    },
  });
  ...
  const cancelLogin = useMutation({
    ...
    onError: (error) => {
      setStartError(error instanceof Error ? error.message : t("localizationAgents.config_Could_not_cancel_the_login_"));
    },
  });
  ...
  return (
    <div className="rounded-md border border-border bg-muted/40 px-3 py-2 space-y-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-medium text-foreground">{t("localizationAgents.config_Sign_in_to_the_environment")}</span>
      ...
      <div role="status" aria-live="polite" className="space-y-2 empty:hidden">
        {isActive && !prompt && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            ...
            <span>{t("stableCore.preparingLogin")}</span>
          </div>
        )}

        {isActive && prompt && (
          <div className="space-y-2">
            <div className="text-(length:--text-micro) text-muted-foreground">{t("stableCore.openAuthentication")}</div>
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("localizationSkills.code221")}</div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("stableCore.authenticationUrl")}</div>
...
function SubmittedBrowserCodeLoginPanel({
  ...
  const submitCode = useMutation({
    ...
    onError: (error) => {
      setStartError(error instanceof Error ? error.message : t("localizationAgents.config_Could_not_submit_the_browser_code_"));
    },
  });
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button
            ...
            onClick={() => startLogin.mutate()}
          >
            {storedToken && !isActive && !isStored ? t("stableCore.logInToReplace") : t("stableCore.logIn")}
          </Button>
        </div>
      </div>
      ...
      <div role="status" aria-live="polite" className="space-y-2 empty:hidden">
        ...
        {isActive && authorizationUrl && !isCompleting && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("stableCore.authorizationUrl")}</div>
            ...
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("localizationAgents.ui185_Browser_code")}</div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ...
    : t("stableCore.noAdapterDefault");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("stableCore.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">{t("stableCore.cheapProfileHelp")}</p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-(length:--text-micro) text-muted-foreground">
          <Trans i18nKey="stableCore.fallsBackToModel" values={{ model: adapterDefaultModel }} components={{ code: <code /> }} />
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-(length:--text-micro) text-amber-500">{t("stableCore.cheapProfileFallback")}</p>
      ) : null}
    </div>
  );
}

...
export function InboxArchiveButton({
  ...
  return (
    <button
      ...
      aria-label={t("pages.secrets.actions.archive", { defaultValue: "Archive" })}
    >
      <Archive className="h-3.5 w-3.5" />
      {t("pages.secrets.actions.archive", { defaultValue: "Archive" })}
    </button>
  );
}
...
export function IssueRow({
  ...
  const productivityReviewIndicator = productivityReview ? (
    <span
      ...
      title={t("localizationIssueLists.productivityReviewTitle", { defaultValue: "Productivity review: {{trigger}}", trigger: productivityReviewTriggerLabel(productivityReview.trigger) })}
      aria-label={t("localizationIssueLists.productivityReviewOpen", { defaultValue: "Productivity review open" })}
    >
  ...
  const parkedBlockerIndicator = hasAssignedBacklogBlocker(issue.blockedBy) ? (
    <Badge variant="outline"
      ...
      title={t("localizationIssueLists.parkedBlockerHelp", { defaultValue: "Blocked by parked work — at least one assigned blocker is in backlog and will not wake its assignee." })}
    >
      <Flag className="h-2.5 w-2.5" aria-hidden />
      {t("localizationIssueLists.parkedBlocker", { defaultValue: "Blocked by parked work" })}
    </Badge>
  ) : null;
  ...
  return (
    ...
    >
      ...
      >
        <span className="sr-only">{t("localizationIssueLists.openIssue", { defaultValue: "Open {{identifier}}: {{title}}", identifier, title: issue.title })}</span>
      </Link>
...
function renderRecoveryChip(
  ...
  return (
    <Badge variant="outline"
      ...
      title={detail
        ? t("localizationIssueLists.recoveryDetailTitle", { defaultValue: "{{label}} — {{detail}}. Open the source task to act.", label, detail })
        : t("localizationIssueLists.recoveryTitle", { defaultValue: "{{label}} — open the source task to act.", label })}
    >

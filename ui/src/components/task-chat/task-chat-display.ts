...
export function taskChatTimestampDisplay(value: unknown): string | undefined {
...
const DISPLAY_KEYS: Readonly<Record<string, string>> = {
  "Reading files": "stableTaskChat.readingFiles",
  "Grepping": "stableTaskChat.grepping",
  "Searching": "stableTaskChat.searching",
  "Fetching the web": "stableTaskChat.fetchingWeb",
  "Delegating": "stableTaskChat.delegating",
...
export function taskThreadMarkerDetailDisplay(value: string): string {
  const planRevision = /^rev (\d+) — see the Plan tab$/.exec(value);
  if (planRevision) return t("stableTaskChat.planRevision", { revision: planRevision[1] });

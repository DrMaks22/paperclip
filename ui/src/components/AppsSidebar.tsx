    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex flex-col gap-1 px-3 py-3 shrink-0">
        ...
        >
          ...
          <span className="truncate">{selectedCompany?.name ?? t("nav.company")}</span>
        </Link>
        <div className="flex items-center gap-2 px-2 py-1">
          <AppWindow className="h-4 w-4 text-muted-foreground shrink-0" />
          <span className="flex-1 truncate text-sm font-bold text-foreground">{t("nav.apps")}</span>
        </div>
      </div>
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide px-3 py-2">
        ...
        <div className="px-3 pb-1 pt-4 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">{t("localizationApps.developer759")}</div>
        <p className="px-3 pb-1.5 text-(length:--text-micro) leading-snug text-muted-foreground/70">{t("localizationApps.advancedSetupForDevelopersMostTeamsNeverOpenT761")}</p>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem to="/apps/connections" label={t("pages.apps.connections.title")} icon={AppWindow} end />

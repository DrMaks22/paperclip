...
export function IssueProperties({
  ...
  }), [t, workspaceCanRunCommands, workspaceCanStartServices, workspaceRuntimeConfig, workspaceRuntimeServices]);
  ...
  const assigneeOptionsTrigger = (() => {
    if (assigneeOverrideLane === "cheap") {
      return <span className="text-sm">{t("stableIssueDetail.cheapModel")}</span>;
    }
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex w-full overflow-hidden rounded-md border border-border" role="radiogroup" aria-label={t("localizationIssueLists.modelLane")}>
          {(["primary", ...(assigneeSupportsCheapLane ? (["cheap"] as const) : ([] as const)), "custom"] as const).map((lane) => (
            ...
            >
              {lane === "primary" ? t("localizationIssueLists.primary") : lane === "cheap" ? t("stableIssueDetail.cheapLane") : t("localizationIssueDetail.ui_Override")}
            </button>
          ))}
        </div>
        {assigneeOverrideLane === "cheap" ? (
          <p className="text-xs text-muted-foreground">
            {t("stableIssueDetail.sends")} <code>modelProfile: "cheap"</code>{" "}
            {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
              ? <>{t("stableIssueDetail.cheapAdapterDefault")} <code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
              : assigneeCheapProfile
                ? <>{t("stableIssueDetail.cheapConfigured")}</>
                : <>{t("stableIssueDetail.cheapFallback")}</>}
          </p>
        ) : null}
  ...
  const watchdogTrigger = issue.watchdog ? (
    ...
    <span className="text-sm text-muted-foreground">{t("localizationIssueDetail.ui_None")}</span>
  );
  ...
  const userAssigneeOptions = [
    ...
    ...otherUserOptions.map((option) => ({
      ...
      label: actualUserLabel(option.id.slice("user:".length)) ?? option.label,
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {otherUserOptions
          .filter((option) => {
            ...
            const label = actualUserLabel(option.id.slice("user:".length)) ?? option.label;
            return `${label} ${option.searchText ?? ""}`.toLowerCase().includes(search.toLowerCase());
          })
          .map((option) => (
            <button
            ...
            >
              <User className="h-3 w-3 shrink-0 text-muted-foreground" />
              {actualUserLabel(option.id.slice("user:".length)) ?? option.label}
            </button>
          ))}
        {sortedAgents
          .filter((agent) => {
  ...
  const propertiesBody = (
    <div>
      <PropertySection title={t("localizationIssueDetail.ui_Triage")} first>
      ...
      <PropertySection title={t("localizationIssueDetail.ui_Relationships")}>
        <PropertyPicker
          inline={inline}
          label={t("localizationIssuePanels.field_parentId")}
          open={parentOpen}
          onOpenChange={(open) => {
            setParentOpen(open);
        ...
        {relatedTasks.length > 0 ? (
          <PropertyRow label={t("localizationIssueDetail.ui_Related_tasks")} wrap>
            <div className="flex flex-wrap items-center gap-1.5">
      ...
      <PropertySection title={t("localizationIssueDetail.ui_Execution")}>
      ...
      {hasWorkspaceRuntimeControls || issue.currentExecutionWorkspace?.branchName || issue.currentExecutionWorkspace?.cwd || issue.executionWorkspaceId ? (
        <PropertySection title={t("pages.agentDetail.workspace")}>
      ...
      <PropertySection title={t("localizationIssueDetail.ui_About")}>
        {originatingActor ? (

    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <SummaryCard
          ...
          value={boundSecrets === 0 ? t("localizationRoutines.none") : t("stableCore.boundSecrets", { count: boundSecrets })}
          hint={t("stableCore.manageBoundSecrets")}
          to={() => navigateToSection("secrets")}
          ariaLabel={t("stableCore.openBoundSecrets", { count: boundSecrets })}
        />
        <SummaryCard
          icon={Play}
            ...
            {secretMessage.entries.map((entry, index) => (
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("localizationRoutines.webhookUrl"), entry.webhookUrl)}>{t("localizationIssueDetail.mdxeditor.createLink.url")}</Button>
                </div>
                <div className="flex items-center gap-2">

...
describe("interaction audience display localization", () => {
  ...
  it("selects full phrases from facts, never from translated actor names", async () => {
    ...
    for (const language of ["ru", "en", "ru"]) {
      await locale(language);
      expect(self.summary).toBe(language === "ru" ? "Ответить может любой участник организации, кроме вас." : "Anyone in the organization except you can respond.");
  ...
  it("localizes the current-user audience in the full rendered card without parsing Вы or You", async () => {
    const interaction = { ...humanOnlyRequestConfirmationInteraction,
      effectiveResolverPolicy: "not_creator" as const, requestedResolverPolicy: "not_creator" as const,
      createdByUserId: "user-current", createdByAgentId: null };
    ...
    for (const language of ["ru", "en", "ru"]) {
      ...
      expect(summary.textContent).toBe(language === "ru" ? "Ответить может любой участник организации, кроме вас." : "Anyone in the organization except you can respond.");
      expect(interaction.createdByUserId).toBe("user-current");
    }
  });
  ...
  it("keeps selected raw options and a full question card refusal through ru → en → ru", async () => {
    ...
    await render(<IssueThreadInteractionCard interaction={interaction} onSubmitInteractionAnswers={submit} />);

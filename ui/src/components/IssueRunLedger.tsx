...
function modelProfileTitle(summary: ModelProfileSummary) {
  const lines = [t("stableCore.requestedProfile", { value: summary.requested })];
  if (summary.applied) lines.push(t("stableCore.appliedProfile", { value: summary.applied }));
  if (summary.configSource) lines.push(t("stableCore.profileSource", { value: summary.configSource }));
  if (summary.fallbackReason) lines.push(t("stableCore.profileFallback", { value: summary.fallbackReason }));
  return lines.join("\n");
}
...
function formatDuration(start: string | Date | null | undefined, end: string | Date | null | undefined) {
  ...
  if (minutes < 60) return seconds > 0 ? t("localizationTaskRuntime.durationMinutesSeconds", { minutes, seconds }) : t("localizationTaskRuntime.durationMinutes", { minutes });
...
function runSummary(run: LedgerRun, agentMap: ReadonlyMap<string, Pick<Agent, "name">>) {
  ...
  if (run.status === "scheduled_retry") return t("localizationTaskRuntime.runRetryFor", { agent: agentName });
...
function stopReasonLabel(run: RunForIssue) {
  ...
  const timeoutText =
    effectiveTimeoutSec && effectiveTimeoutSec > 0 ? t("localizationTaskRuntime.timeoutSeconds", { count: effectiveTimeoutSec }) : null;
  ...
  if (stopReason === "max_turns_exhausted" || stopReason === "turn_limit_exhausted") return t("localizationTaskRuntime.ui_max_turns_exhausted_3xo7wa");
  ...
  if (stopReason === "unmanaged_background_task_stopped") return t("localizationTaskRuntime.ui_unmanaged_background_task_stopped_ro89fb");
  if (stopReason === "adapter_failed") return t("localizationTaskRuntime.ui_adapter_failed_12m9vap");
  if (stopReason === "completed") return timeoutText ? t("localizationTaskRuntime.completedDetail", { timeout: timeoutText }) : t("localizationTaskRuntime.completed");
  return timeoutText;
}
...
function formatSilenceAge(ms: number | null | undefined) {
  ...
  if (totalMinutes < 60) return t("localizationTaskRuntime.silenceMinutes", { count: totalMinutes });
    ...
    <section className="space-y-3" aria-label={t("localizationTaskRuntime.ui_Task_run_ledger_1wkqi76")}>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-sm font-medium text-muted-foreground">{t("localizationTaskRuntime.ui_Run_ledger_qqlilj")}</h3>
              ...
              >
                  ...
                  return (
                    <p className="min-w-0 break-words text-(length:--text-micro) leading-5 text-amber-700 dark:text-amber-300">
                      {profile.requested === "cheap"
                        ? t("stableCore.cheapProfileFellBack")
                ...
                {run.nextAction ? (
                  <div className="min-w-0 rounded-md bg-accent/40 px-2 py-1.5 text-xs leading-5">
                    <span className="font-medium text-foreground">{t("localizationTaskRuntime.ui_Next_action_n7deyw")}</span>

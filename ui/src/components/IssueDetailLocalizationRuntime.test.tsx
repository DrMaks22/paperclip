...
describe("task detail live localization", () => {
  ...
  it("keeps edited full-card checkbox selections while translating count constraints", async () => {
    ...
    await render(<IssueThreadInteractionCard interaction={interaction} onAcceptInteraction={accept} />);
    const option = host.querySelector<HTMLButtonElement>('[role="checkbox"][aria-label="EU West (Ireland)"]')!;
    ...
    for (const locale of ["ru", "en", "ru"]) {
      ...
      expect(host.textContent).toContain(locale === "ru" ? "Выбрано вариантов: 3 из 12" : "3 of 12 options selected");
      expect(host.textContent).toContain(locale === "ru" ? "Выберите от 2 до 3." : "Pick 2-3.");
    ...
    expect(accept).toHaveBeenCalledWith(interaction, undefined, ["us-west", "us-east", "eu-west"]);
  });

  it("refreshes display-only monitor labels and options without changing scheduling or canonical formatters", async () => {

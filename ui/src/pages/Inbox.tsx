...
function runFailureMessage(run: HeartbeatRun): string {
  return firstNonEmptyLine(run.error)
    ...
    ?? t("pages.inbox.runExitedWithError", { defaultValue: "Run exited with an error." });
}
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    return t("pages.inbox.agentJoinRequest", {
      defaultValue: "Agent join request{{name}}",
      name: joinRequest.agentName ? `: ${joinRequest.agentName}` : "",
    });
  }
  ...
  return t("pages.inbox.humanJoinRequest", { defaultValue: "Human join request" });
}
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{t("pages.inbox.failedRun", {
                  defaultValue: "Failed run{{name}}",
                  name: linkedAgentName ? ` — ${linkedAgentName}` : "",
                })}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying
              ? t("pages.inbox.retrying", { defaultValue: "Retrying…" })
              : t("pages.inbox.retry", { defaultValue: "Retry" })}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              {requesterName ? <span>{t("pages.inbox.requestedBy", { defaultValue: "requested by {{name}}", name: requesterName })}</span> : null}
              <span>{t("pages.inbox.updated", { defaultValue: "updated {{time}}", time: timeAgo(approval.updatedAt) })}</span>
            </span>
          </span>
        </Link>
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("pages.inbox.requestedFromIp", {
                defaultValue: "requested {{time}} from IP {{ip}}",
                time: timeAgo(joinRequest.createdAt),
                ip: joinRequest.requestIp,
              })}</span>
              {joinRequest.adapterType && <span>{t("pages.inbox.adapter", { defaultValue: "adapter: {{type}}", type: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board"
          ? t("pages.inbox.board", { defaultValue: "Board" })
          : t("filter.me"),
        kind: "user",
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToApprove", { defaultValue: "Failed to approve" }));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToReject", { defaultValue: "Failed to reject" }));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToApproveJoin", { defaultValue: "Failed to approve join request" }));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToRejectJoin", { defaultValue: "Failed to reject join request" }));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToArchiveTask", { defaultValue: "Failed to archive task" }));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("pages.inbox.failedToUndoArchive", { defaultValue: "Failed to undo inbox archive" }));
          ...
          <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
            <PageTabBar
              items={[
                {
                  value: "mine",
                  label: t("pages.inbox.tabMine", { defaultValue: "Mine" }),
                },
                {
                  value: "recent",
                  label: t("pages.inbox.tabRecent", { defaultValue: "Recent" }),
                },
                { value: "unread", label: t("pages.inbox.tabUnread", { defaultValue: "Unread" }) },
                { value: "blocked", label: t("pages.inbox.tabBlocked", { defaultValue: "Blocked" }) },
                { value: "all", label: t("pages.inbox.tabAll", { defaultValue: "All" }) },
              ]}
            />
          </Tabs>
            ...
            <>
              <Button
                ...
                title={nestingEnabled
                  ? t("pages.inbox.disableNesting", { defaultValue: "Disable parent-child nesting" })
                  : t("pages.inbox.enableNesting", { defaultValue: "Enable parent-child nesting" })}
              >
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ["none", t("status.none")],
                      ["type", t("pages.inbox.groupByType", { defaultValue: "Type" })],
                      ["assignee", t("pages.inbox.groupByAssignee", { defaultValue: "Responsible" })],
                      ["project", t("pages.inbox.groupByProject", { defaultValue: "Project" })],
                      ...(isolatedWorkspacesEnabled ? ([["workspace", t("pages.inbox.groupByWorkspace", { defaultValue: "Workspace" })]] as const) : []),
                ...
                <>
                  ...
                  >
                    {markAllReadMutation.isPending
                      ? t("pages.inbox.marking", { defaultValue: "Marking…" })
                  ...
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          {t("pages.inbox.markAllAsReadDescription", {
                            defaultValue: "This will mark {{count}} unread {{noun}} as read.",
                            count: unreadIssueIds.length,
                            noun: unreadIssueIds.length === 1
                              ? t("pages.inbox.item", { defaultValue: "item" })
                              : t("pages.inbox.items", { defaultValue: "items" }),
                          })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {t("pages.inbox.subtaskCount", {
                            defaultValue: "({{count}} {{noun}})",
                            count: childCount,
                            noun: childCount === 1
                              ? t("pages.inbox.subtask", { defaultValue: "sub-task" })
                              : t("pages.inbox.subtasks", { defaultValue: "sub-tasks" }),
                          })}
                        </span>
                      ) : undefined}
                      ...
                      >
                        ...
                        <span className="shrink-0 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived"
                            ? t("status.archived")
                            : t("pages.inbox.otherResults", { defaultValue: "Other results" })}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("pages.inbox.newTaskIn", { defaultValue: "New task in {{group}}", group: group.label })}
                              aria-label={t("pages.inbox.newTaskIn", { defaultValue: "New task in {{group}}", group: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {dashboard!.agents.error === 1
                        ? t("pages.inbox.agentHasErrors", { defaultValue: "agent has errors" })
                        : t("pages.inbox.agentsHaveErrors", { defaultValue: "agents have errors" })}
                    </span>
                  </Link>
                  <button
                    type="button"
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("pages.inbox.budgetAt", { defaultValue: "Budget at" })}{" "}
                      <span className="font-medium">{dashboard!.costs.monthUtilizationPercent}%</span>{" "}
                      {t("pages.inbox.utilizationThisMonth", { defaultValue: "utilization this month" })}
                    </span>
                  </Link>
                  <button
                    type="button"

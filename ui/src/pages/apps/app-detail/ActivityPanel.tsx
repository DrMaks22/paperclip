...
function RecentActivity({
  ...
  return (
    <section className="space-y-2">
      <div>
        <h2 className="text-sm font-bold text-foreground">{t("pages.userProfile.recentActivity")}</h2>
      </div>
      {loading ? (
...
function lifecycleLinkLabel(event: ToolConnectionLifecycleEvent): string {
  return event.type === "actions_quarantined" ? t("stableApps.activity.reviewSetup") : t("stableApps.activity.viewSetup");
}

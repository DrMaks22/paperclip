...
export function ReviewQueueCard({
  ...
  if (items.length === 0) {
    ...
    return (
      <div className="rounded-xl border border-border bg-card p-5 text-sm text-muted-foreground">{t("pages.apps.review.empty")}</div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-amber-500/40 bg-amber-500/[0.07] p-4">
      ...
      {preview ? (
        ...
        <p className="mt-1 text-sm text-muted-foreground">{t("pages.apps.review.requestExplanation")}</p>
      )}

      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Button size="sm" onClick={() => allowOnce.mutate()} disabled={busy}>
          {resolving === "allow" ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Check className="mr-1.5 h-3.5 w-3.5" />}{t("pages.apps.review.allowOnce")}</Button>
        <Button size="sm" variant="outline" onClick={() => alwaysAllow.mutate()} disabled={busy}>
          {resolving === "always" ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}{t("pages.apps.review.alwaysAllow")}</Button>

...
export function ProfilesIndex({
  ...
  const duplicate = useMutation({
    mutationFn: (profile: ToolProfileWithDetails) =>
      toolsApi.duplicateProfile(profile.id, { name: t("localizationTools.profileCopyParenthesized", { name: profile.name }) }),
    onSuccess: () => {
    ...
    onError: (error: unknown) =>
      pushToast({ title: t("localizationTools.couldNotDuplicate55"), body: errorBody(error), tone: "error" }),
  });
  ...
  const archive = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("localizationTools.profileArchived56"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("localizationTools.couldNotArchive57"), body: errorBody(error), tone: "error" }),
  });
  ...
  const restore = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("localizationTools.profileRestored58"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("localizationTools.couldNotRestore59"), body: errorBody(error), tone: "error" }),
  });
  ...
  const header = (
    <ToolsPageHeader
      ...
      description={t("localizationTools.decideWhichToolsYourAgentsCanUseBuildAProfile63")}
      actions={
        <>
          <Button variant="outline" onClick={() => setResolverOpen(true)}>
            <ShieldCheck className="mr-1.5 h-4 w-4" />{t("localizationTools.checkAnAgentSAccess64")}</Button>
          <Button onClick={() => navigate(newProfileHref())}>
            <Plus className="mr-1.5 h-4 w-4" />{t("localizationTools.newProfile65")}</Button>
        </>
      }
    />
  );
    ...
    <Sheet open={resolverOpen} onOpenChange={setResolverOpen}>
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle>{t("localizationTools.checkAnAgentSAccess64")}</SheetTitle>
          <SheetDescription>{t("localizationTools.seeExactlyWhichToolsAnAgentCanUseRightNowAndW66")}</SheetDescription>
        </SheetHeader>
  ...
  if (profiles.isLoading) {
    return (
      <div className="space-y-5">
        {header}
        <LoadingState label={t("localizationApps.loadingProfiles294")} />
      </div>
    );
  }
  if (profiles.isError) {
    ...
    <div className="space-y-5">
      ...
      <div className="inline-flex rounded-md border border-border p-0.5">
        {(["active", "archived"] as const).map((key) => (
          ...
          >
            {key === "active" ? t("status.active") : t("status.archived")}
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        statusFilter === "archived" ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">{t("localizationTools.noArchivedProfiles69")}</div>
        ) : (
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
                <th className="px-3 py-2 font-medium">{t("pages.profile.title")}</th>
                <th className="px-3 py-2 font-medium">{t("localizationTools.allows71")}</th>
                <th className="px-3 py-2 font-medium">{t("localizationTools.assignedTo72")}</th>
                <th className="px-3 py-2 font-medium">{t("pages.apps.connections.columnStatus")}</th>
                <th className="px-3 py-2 font-medium">{t("pages.cases.columnUpdated")}</th>
                      ...
                      <span className="inline-flex items-center gap-2">
                        ...
                        {(profile.newToolsPendingCount ?? 0) > 0 ? (
                          <Badge variant="outline" className="border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-200">
                            {t("localizationTools.newToolsCount", { count: profile.newToolsPendingCount })}
                          </Badge>
                        ) : null}
                      </span>
                    </td>
                    <td className="px-3 py-1.5">
                      {assigned.unassigned ? (
                        <span className="text-muted-foreground">
                          {assigned.text}
                          <span className="ml-1 text-xs text-muted-foreground/70">{t("localizationTools.doesNotChangeAccess")}</span>
                        </span>
                      ) : (
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("localizationTools.profileActions77")}
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onSelect={onDuplicate}>{t("localizationTools.duplicate79")}</DropdownMenuItem>
        {onRestore ? (
          <DropdownMenuItem onSelect={onRestore}>
            <ArchiveRestore className="mr-1.5 h-4 w-4" />{t("pages.agentDetail.restore")}</DropdownMenuItem>
        ) : (
          <DropdownMenuItem onSelect={onArchive}>{t("localizationRoutines.archive")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onDelete} className="text-destructive focus:text-destructive">{t("pages.agentDetail.delete")}</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-6">
      <div className="mb-4 max-w-2xl">
        <h3 className="text-base font-semibold text-foreground">{t("localizationTools.createYourFirstAccessProfile83")}</h3>
        <p className="text-sm text-muted-foreground">{t("localizationTools.pickAStartingPointYouCanFineTuneExactlyWhichT84")}</p>
      </div>

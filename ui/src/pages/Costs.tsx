  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("pages.costs.financeLedger")}</CardTitle>
        <CardDescription>{t("pages.costs.financeLedgerDescription")}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("pages.costs.debits")}
          value={formatCents(debitCents)}
          subtitle={t("localizationCosts.rangeEvents", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("pages.costs.credits")}
          value={formatCents(creditCents)}
          subtitle={t("pages.costs.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("pages.costs.net")}
          value={formatCents(netCents)}
          subtitle={t("pages.costs.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("pages.costs.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("pages.costs.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("pages.costs.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("pages.costs.allBillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("pages.costs.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{t("pages.costs.pageDescription")}</p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("pages.costs.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("pages.costs.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("localizationCosts.requestTokens", { count: inferenceTokenTotal, tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("pages.costs.budget")}
              value={activeBudgetIncidents.length > 0 ? (activeBudgetIncidents.length).toLocaleString(i18n.resolvedLanguage) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent.toLocaleString(i18n.resolvedLanguage, { maximumFractionDigits: 20 })}%`
                  : t("localizationCosts.openBudget")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? [t("localizationCosts.pausedAgents", { count: budgetData?.pausedAgentCount ?? 0 }), t("localizationCosts.pausedProjects", { count: budgetData?.pausedProjectCount ?? 0 })].join(" · ")
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("localizationCosts.spendOfBudget", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("pages.costs.noMonthlyCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("pages.costs.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("localizationCosts.debitsCredits", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("pages.costs.financeEvents")}
              value={(financeData?.summary.eventCount ?? 0).toLocaleString(i18n.resolvedLanguage)}
              subtitle={t("localizationCosts.estimatedInRange", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("pages.costs.tabOverview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("pages.costs.tabBudgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("pages.costs.tabProviders")}</TabsTrigger>
          <TabsTrigger value="billers">{t("pages.costs.tabBillers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("pages.costs.tabFinance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("pages.costs.selectDateRange")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.inferenceLedger")}</CardTitle>
                    <CardDescription>{t("pages.costs.inferenceLedgerDescription")}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("localizationCosts.budgetAmount", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("pages.costs.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("pages.costs.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("localizationCosts.monthlyBudgetUsed", { percent: spendData.summary.utilizationPercent.toLocaleString(i18n.resolvedLanguage, { maximumFractionDigits: 20 }) })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.byAgent")}</CardTitle>
                    <CardDescription>{t("pages.costs.byAgentDescription")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("pages.costs.noCostEvents")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("localizationCosts.inputOutputTokens", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("localizationCosts.apiRuns", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("localizationCosts.subscriptionRuns", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("localizationAgents.tokenTotal", { value: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byProject")}</CardTitle>
                      <CardDescription>{t("pages.costs.byProjectDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("pages.costs.noProjectCosts")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("localizationCosts.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("pages.costs.financeTimelineEmpty")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("pages.costs.budgetControlPlane")}</CardTitle>
                  <CardDescription>{t("pages.costs.budgetControlPlaneDescription")}</CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("pages.costs.activeIncidents")}
                    value={(activeBudgetIncidents.length).toLocaleString(i18n.resolvedLanguage)}
                    subtitle={t("pages.costs.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("pages.costs.pendingApprovals")}
                    value={(budgetData?.pendingApprovalCount ?? 0).toLocaleString(i18n.resolvedLanguage)}
                    subtitle={t("pages.costs.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedAgents")}
                    value={(budgetData?.pausedAgentCount ?? 0).toLocaleString(i18n.resolvedLanguage)}
                    subtitle={t("pages.costs.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedProjects")}
                    value={(budgetData?.pausedProjectCount ?? 0).toLocaleString(i18n.resolvedLanguage)}
                    subtitle={t("pages.costs.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("pages.costs.activeIncidentsDescription")}</p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
              ...
              <div className="space-y-5">
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(scopeType === "company" ? "stableCosts.organizationBudgets" : `localizationCosts.budgets_${scopeType}`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("pages.costs.companyPolicy")
                            : scopeType === "agent"
                              ? t("pages.costs.agentPolicy")
                              : t("pages.costs.projectPolicy")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">{t("pages.costs.noBudgetPolicies")}</CardContent>
                  </Card>
                ) : null}
              </div>
            </>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("pages.costs.noCostEventsInPeriod")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("pages.costs.noBillableEvents")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byBiller")}</CardTitle>
                      <CardDescription>{t("pages.costs.byBillerDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("pages.costs.noFinanceEvents")}</p>
                      ) : (

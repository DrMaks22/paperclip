  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("pages.costs.financeLedger", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
          {t("pages.costs.financeLedgerDescription", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("pages.costs.debits", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={t("pages.costs.totalEventsInRange", { defaultValue_one: "{{count}} total event in range", defaultValue_other: "{{count}} total events in range", count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("pages.costs.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("pages.costs.creditsSubtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("pages.costs.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("pages.costs.netSubtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("pages.costs.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("pages.costs.estimatedSubtitle", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("pages.costs.allProviders", { defaultValue: "All providers" })}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("pages.costs.allBillers", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("pages.costs.selectCompany", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("nav.costs")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("pages.costs.pageDescription", { defaultValue: "Inference spend, platform fees, credits, and live quota windows." })}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("pages.costs.to", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("pages.costs.inferenceSpend", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("pages.costs.tokensAcrossRequestScoped", { defaultValue: "{{countFormatted}} tokens across request-scoped events", count: inferenceTokenTotal, countFormatted: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("pages.costs.budget", { defaultValue: "Budget" })}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("pages.costs.agentsAndProjectsPaused", { defaultValue: "{{agents}} agents paused · {{projects}} projects paused", agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("pages.costs.spendOfBudget", { defaultValue: "{{spend}} of {{budget}}", spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("pages.costs.noMonthlyCap", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
              label={t("pages.costs.financeNet", { defaultValue: "Finance net" })}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("pages.costs.debitsCredits", { defaultValue: "{{debits}} debits · {{credits}} credits", debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("pages.costs.financeEvents", { defaultValue: "Finance events" })}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("pages.costs.estimatedInRange", { defaultValue: "{{countFormatted}} estimated in range", count: financeData?.summary.estimatedDebitCents ?? 0, countFormatted: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("pages.costs.tabOverview", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="budgets">{t("pages.costs.tabBudgets", { defaultValue: "Budgets" })}</TabsTrigger>
          <TabsTrigger value="providers">{t("pages.costs.tabProviders", { defaultValue: "Providers" })}</TabsTrigger>
          <TabsTrigger value="billers">{t("pages.costs.tabBillers", { defaultValue: "Billers" })}</TabsTrigger>
          <TabsTrigger value="finance">{t("pages.costs.tabFinance", { defaultValue: "Finance" })}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("pages.costs.selectDateRange", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.inferenceLedger", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      {t("pages.costs.inferenceLedgerDescription", { defaultValue: "Request-scoped inference spend for the selected period." })}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("pages.costs.budgetValue", { defaultValue: "Budget {{countFormatted}}", count: spendData.summary.budgetCents, countFormatted: formatCents(spendData.summary.budgetCents) })
                            : t("pages.costs.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("pages.costs.usage", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("pages.costs.percentOfMonthlyBudget", { defaultValue: "{{percent}}% of monthly budget consumed in this range.", percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.byAgent", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("pages.costs.byAgentDescription", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("pages.costs.noCostEvents", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("pages.costs.inOutTokens", { defaultValue: "in {{inTokens}} · out {{outTokens}}", inTokens: formatTokens(row.inputTokens + row.cachedInputTokens), outTokens: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? t("pages.costs.apiRuns", { defaultValue: "{{count}} api", count: row.apiRunCount }) : t("pages.costs.apiRuns", { defaultValue: "{{count}} api", count: 0 })}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? t("pages.costs.subscriptionRuns", { defaultValue: "{{count}} subscription", count: row.subscriptionRunCount })
                                      : t("pages.costs.subscriptionRuns", { defaultValue: "{{count}} subscription", count: 0 })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("pages.costs.tokensAbbrev", { defaultValue: "{{countFormatted}} tok", count: modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens, countFormatted: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byProject", { defaultValue: "By project" })}</CardTitle>
                      <CardDescription>{t("pages.costs.byProjectDescription", { defaultValue: "Run costs attributed through project-linked tasks." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("pages.costs.noProjectCosts", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("pages.costs.unattributed", { defaultValue: "Unattributed" })}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("pages.costs.financeTimelineEmpty", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("pages.costs.budgetControlPlane", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                    {t("pages.costs.budgetControlPlaneDescription", { defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers." })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("pages.costs.activeIncidents", { defaultValue: "Active incidents" })}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("pages.costs.activeIncidentsSubtitle", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("pages.costs.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("pages.costs.pendingApprovalsSubtitle", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("pages.costs.pausedAgentsSubtitle", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("pages.costs.pausedProjectsSubtitle", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("pages.costs.activeIncidents")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("pages.costs.activeIncidentsDescription", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ...
                            : scopeType === "agent"
                              ? t("pages.costs.agentPolicy", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("pages.costs.projectPolicy", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("pages.costs.noBudgetPolicies", { defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control." })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("pages.costs.noCostEventsInPeriod", { defaultValue: "No cost events in this period." })}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("pages.costs.noBillableEvents", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byBiller", { defaultValue: "By biller" })}</CardTitle>
                      <CardDescription>{t("pages.costs.byBillerDescription", { defaultValue: "Account-level financial events grouped by who charged or credited them." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("pages.costs.noFinanceEvents", { defaultValue: "No finance events yet." })}</p>
                      ) : (

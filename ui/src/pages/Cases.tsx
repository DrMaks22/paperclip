...
const STATUS_FILTER_OPTIONS: { value: CaseStatus; label: string; labelKey: string }[] = [
  { value: "draft", label: "Draft", labelKey: "pages.cases.statusDraft" },
  { value: "in_progress", label: "In progress", labelKey: "pages.cases.statusInProgress" },
  { value: "in_review", label: "In review", labelKey: "pages.cases.statusInReview" },
  { value: "approved", label: "Approved", labelKey: "pages.cases.statusApproved" },
  { value: "done", label: "Done", labelKey: "pages.cases.statusDone" },
  { value: "cancelled", label: "Cancelled", labelKey: "pages.cases.statusCancelled" },
];
...
const CASE_COLUMN_LABELS: Record<CaseColumn, { label: string; labelKey: string }> = {
  id: { label: "ID", labelKey: "pages.cases.columnId" },
  key: { label: "Key", labelKey: "pages.cases.columnKey" },
  title: { label: "Title", labelKey: "pages.cases.columnTitle" },
  status: { label: "Status", labelKey: "pages.cases.columnStatus" },
  updated: { label: "Updated", labelKey: "pages.cases.columnUpdated" },
  created: { label: "Created at", labelKey: "pages.cases.columnCreated" },
  type: { label: "Type", labelKey: "pages.cases.columnType" },
  project: { label: "Project", labelKey: "pages.cases.columnProject" },
  parent: { label: "Parent case", labelKey: "pages.cases.columnParent" },
};
const CASE_SORT_LABELS: Record<CaseSortField, { label: string; labelKey: string }> = {
  updated: { label: "Last updated", labelKey: "pages.cases.sortUpdated" },
  created: { label: "Created at", labelKey: "pages.cases.sortCreated" },
  title: { label: "Title", labelKey: "pages.cases.sortTitle" },
  status: { label: "Status", labelKey: "pages.cases.sortStatus" },
  id: { label: "ID", labelKey: "pages.cases.sortId" },
  type: { label: "Type", labelKey: "pages.cases.sortType" },
  project: { label: "Project", labelKey: "pages.cases.sortProject" },
};
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("pages.cases.changeStatus", { defaultValue: "Change case status" })}
          onClick={(event) => {
  ...
  return (
    <span className="grid min-w-0 flex-1 items-center gap-2" style={{ gridTemplateColumns: caseTrailingGridTemplate(columns) }}>
      {columns.map((column) => {
        if (column === "id") {
          return (
            <CaseCopyableToken
              ...
              label={t("pages.cases.copyCaseIdLabel", { defaultValue: "case ID" })}
        ...
        if (column === "key") {
          return row.key ? (
            <CaseCopyableToken
              key={column}
              value={row.key}
              label={t("pages.cases.copyCaseKeyLabel", { defaultValue: "case key" })}
              className="font-mono text-xs text-muted-foreground"
              stopPropagation
            />
          ) : (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">{t("pages.cases.none", { defaultValue: "None" })}</span>
          );
        }
        if (column === "title") {
          return (
                  ...
                  {childCount > 0 ? (
                    <button
                      ...
                      aria-label={treeCollapsed
                        ? t("pages.cases.expandCase", { defaultValue: "Expand {{title}}", title: row.title })
                        : t("pages.cases.collapseCase", { defaultValue: "Collapse {{title}}", title: row.title })}
        ...
        if (column === "project") {
          return <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">{projectName ?? t("pages.cases.noProject", { defaultValue: "No project" })}</span>;
        }
        if (column === "parent") {
          return (
            <span key={column} className="min-w-0 truncate font-mono text-xs text-muted-foreground">
              {row.parentCaseId ? t("pages.cases.parent", { defaultValue: "Parent" }) : t("pages.cases.none", { defaultValue: "None" })}
            </span>
          );
        }
        if (column === "created") {
        ...
        <span className="grid min-w-0 flex-1 items-center gap-2" style={{ gridTemplateColumns: caseTrailingGridTemplate(trailingColumns) }}>
          {trailingColumns.map((column) => (
            <span key={column} className={cn("truncate", (column === "updated" || column === "created") && "text-right")}>
              {t(CASE_COLUMN_LABELS[column].labelKey, { defaultValue: CASE_COLUMN_LABELS[column].label })}
            </span>
          ))}
        </span>
      ) : null}
        ...
        <IssueGroupHeader
          ...
          trailing={(
            <span className="text-xs text-muted-foreground tabular-nums">
              {t("pages.cases.caseCount", { defaultValue_one: "{{count}} case", defaultValue_other: "{{count}} cases", count })}
            </span>
          )}
        />
      </div>
...
function CaseColumnPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={Columns3} title={t("pages.cases.columns", { defaultValue: "Columns" })} active={!sameStringSet([...visibleColumns], DEFAULT_CASE_COLUMNS)}>
      <PopoverContent align="end" className="w-(--sz-300px) p-1.5">
        <div className="px-2 pb-1 pt-1.5">
          <div className="text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("pages.cases.desktopCaseRows", { defaultValue: "Desktop case rows" })}
          </div>
          <div className="text-sm font-medium text-foreground">{t("pages.cases.chooseVisibleColumns", { defaultValue: "Choose visible columns" })}</div>
        </div>
        <div className="space-y-0.5">
          {CASE_COLUMN_ORDER.map((column) => (
            ...
            >
              <span>{t(CASE_COLUMN_LABELS[column].labelKey, { defaultValue: CASE_COLUMN_LABELS[column].label })}</span>
        ...
        <div className="mt-1 border-t border-border pt-1">
          <button
            ...
            onClick={onReset}
          >
            {t("pages.cases.resetDefaults", { defaultValue: "Reset defaults" })}
          </button>
        </div>
      </PopoverContent>
    </CaseToolbarButton>
...
function CaseSortPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={ArrowUpDown} title={t("pages.cases.sort", { defaultValue: "Sort" })} active={sortField !== "updated" || sortDir !== "desc"}>
      <PopoverContent align="end" className="w-48 p-2">
        {(Object.keys(CASE_SORT_LABELS) as CaseSortField[]).map((field) => (
          ...
          >
            <span>{t(CASE_SORT_LABELS[field].labelKey, { defaultValue: CASE_SORT_LABELS[field].label })}</span>
            {sortField === field ? (
...
function CasesEmptyHero() {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-16 text-center">
      ...
      <h2 className="text-lg font-semibold">{t("pages.cases.emptyTitle", { defaultValue: "No cases yet" })}</h2>
      <p className="text-sm text-muted-foreground">
        {t("pages.cases.emptyDescription", { defaultValue: "Cases are durable work products — blog posts, tweet storms, docs pages — that tasks create and iterate on. In v1 they're created by agents, not from the UI." })}
      </p>
      <div className="w-full space-y-2 rounded-lg border border-border bg-muted/50 p-4 text-left">
        <p className="text-sm font-medium">{t("pages.cases.emptySkillPrompt", { defaultValue: "To start creating cases, add this to a skill:" })}</p>
...
{slug, target_audience, publish_url} and key <release>/<slug>."`}
        ...
        <p className="text-xs text-muted-foreground">
          {t("pages.cases.emptySkillRefPrefix", { defaultValue: "See the paperclip skill" })} → <code className="font-mono">references/cases.md</code>{t("pages.cases.emptySkillRefSuffix", { defaultValue: " for the API." })}
        </p>
      </div>
      <p className="text-xs text-muted-foreground">
        {t("pages.cases.emptyFeatureGatePrefix", { defaultValue: "Feature is gated by the " })}<code className="font-mono">enableCases</code>{t("pages.cases.emptyFeatureGateSuffix", { defaultValue: " experimental flag (Settings → Experimental)." })}
      </p>
    </div>
  );
}
...
export function Cases() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.cases.title", { defaultValue: "Cases" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const groupedRows = useMemo((): CaseGroupedRows[] => {
    ...
    for (const c of sorted) {
      ...
      else key = c.projectId
        ? projectName.get(c.projectId) ?? "Unknown project"
        : "No project";
    ...
    return [...map.entries()]
      ...
      .map(([key, rows]) => ({
        key,
        // Keep stable grouping/collapse keys and user-provided project names raw.
        // Only groups made entirely from a missing-project fallback get a display label.
        label: viewState.groupBy === "project" && rows.every((row) => !row.projectId)
          ? t("pages.cases.noProject", { defaultValue: "No project" })
          : viewState.groupBy === "project" && rows.every((row) => row.projectId && !projectName.has(row.projectId))
            ? t("pages.cases.unknownProject", { defaultValue: "Unknown project" })
            : key,
        rows,
      }));
  ...
  if (viewState.search.trim()) activeFilters.push({ key: "search", label: t("pages.cases.filterSearch", { defaultValue: "Search" }), value: viewState.search.trim() });
  if (viewState.typeFilters.length > 0) {
    activeFilters.push({ key: "type", label: t("pages.cases.filterType", { defaultValue: "Type" }), value: viewState.typeFilters.join(", ") });
  }
  if (!usesDefaultStatusFilter) {
    activeFilters.push({
      key: "status",
      label: t("pages.cases.filterStatus", { defaultValue: "Status" }),
      value: viewState.statusFilters.length === CASE_STATUSES.length
        ? t("pages.cases.filterStatusAll", { defaultValue: "All" })
        : viewState.statusFilters
          .map((status) => {
            const option = STATUS_FILTER_OPTIONS.find((candidate) => candidate.value === status);
            return option ? t(option.labelKey, { defaultValue: option.label }) : status;
          })
          .join(", "),
    });
  }
  if (viewState.projectFilters.length > 0) {
    activeFilters.push({
      key: "project",
      label: t("pages.cases.filterProject", { defaultValue: "Project" }),
      value: viewState.projectFilters
        .map((projectId) => projectId === ALL
          ? t("pages.cases.noProject", { defaultValue: "No project" })
          : projectName.get(projectId) ?? t("pages.cases.filterProject", { defaultValue: "Project" }))
        .join(", "),
    });
  }
  if (viewState.labelFilter !== ALL) {
    const name = (labelsQuery.data ?? []).find((l) => l.id === viewState.labelFilter)?.name ?? t("pages.cases.filterLabelNameFallback", { defaultValue: "Label" });
    activeFilters.push({ key: "label", label: t("pages.cases.filterLabel", { defaultValue: "Label" }), value: name });
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold">{t("pages.cases.title", { defaultValue: "Cases" })}</h1>
          <Badge variant="secondary">{t("pages.cases.experimental", { defaultValue: "Experimental" })}</Badge>
        </div>
      </div>
      ...
      {noCasesAtAll ? (
        ...
        <>
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="relative w-48 sm:w-64 md:w-80">
              ...
              <Input
                ...
                placeholder={t("pages.cases.searchPlaceholder", { defaultValue: "Search cases..." })}
                className="pl-7 text-xs sm:text-sm"
                aria-label={t("pages.cases.searchLabel", { defaultValue: "Search cases" })}
            ...
            <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
              <Button
                ...
                title={viewState.treeView
                  ? t("pages.cases.showFlatList", { defaultValue: "Show flat case list" })
                  : t("pages.cases.showTree", { defaultValue: "Show parent/children tree" })}
                aria-label={viewState.treeView
                  ? t("pages.cases.showFlatList", { defaultValue: "Show flat case list" })
                  : t("pages.cases.showTree", { defaultValue: "Show parent/children tree" })}
              ...
              <CaseToolbarButton icon={Filter} title={t("pages.cases.filters", { defaultValue: "Filters" })} active={hasActiveFilters}>
                <PopoverContent align="end" className="w-72 p-3">
                  <div className="grid gap-3">
                    <FilterField label={t("pages.cases.filterType", { defaultValue: "Type" })}>
                      <div className="max-h-40 overflow-y-auto">
                        {distinctTypes.length === 0 ? (
                          <p className="px-1 py-1 text-xs text-muted-foreground">{t("pages.cases.noTypesYet", { defaultValue: "No types yet" })}</p>
                    ...
                    <FilterField label={t("pages.cases.filterStatus", { defaultValue: "Status" })}>
                      <div>
                        {STATUS_FILTER_OPTIONS.map((option) => (
                          <FilterCheckboxRow
                            key={option.value}
                    ...
                    <FilterField label={t("pages.cases.filterProject", { defaultValue: "Project" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("pages.cases.noProject", { defaultValue: "No project" })}
                    ...
                    <FilterField label={t("pages.cases.filterLabel", { defaultValue: "Label" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("pages.cases.allLabels", { defaultValue: "All labels" })}
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={clearFilters} disabled={!hasActiveFilters}>
                      {t("pages.cases.clearFilters", { defaultValue: "Clear filters" })}
                    </Button>
                  </div>
                </PopoverContent>
              </CaseToolbarButton>
              ...
              <CaseToolbarButton icon={Layers} title={t("pages.cases.group", { defaultValue: "Group" })} active={viewState.groupBy !== "type"}>
                <PopoverContent align="end" className="w-44 p-2">
                  {(["type", "project", "status", "none"] as const).map((value) => {
                    const label = value === "type"
                      ? t("pages.cases.groupByType", { defaultValue: "Type" })
                      : value === "project"
                        ? t("pages.cases.groupByProject", { defaultValue: "Project" })
                        : value === "status"
                          ? t("pages.cases.groupByStatus", { defaultValue: "Status" })
                          : t("pages.cases.groupByNone", { defaultValue: "None" });
                    return (
                      <button
                        key={value}
                        type="button"
          ...
          {filtered.length === 0 ? (
            <EmptyState
              icon={SearchX}
              message={t("pages.cases.noMatchMessage", { defaultValue: "No cases match these filters." })}
              action={t("pages.cases.clearFilters", { defaultValue: "Clear filters" })}

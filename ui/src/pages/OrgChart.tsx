...
export function OrgChart() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.org.title") }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Network} message={t("pages.orgChart.selectCompany")} />;
  }
  ...
  if (orgTree && orgTree.length === 0) {
    return <EmptyState icon={Network} message={t("pages.agents.noOrg")} />;
  }
        ...
        {showImport && (
          <Link to="/company/import">
            <Button variant="outline" size="sm">
              <Upload className="mr-1.5 h-3.5 w-3.5" />{t("pages.orgChart.importCompany")}</Button>
          </Link>
        )}
        {showExport && (
          <Link to="/company/export">
            <Button variant="outline" size="sm">
              <Download className="mr-1.5 h-3.5 w-3.5" />{t("pages.orgChart.exportCompany")}</Button>
          </Link>
        )}
      </div>
      <div
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-10 flex flex-col gap-1.5">
          <button
            ...
            title={t("pages.orgChart.zoomIn")}
            aria-label={t("pages.orgChart.zoomIn")}
          >
          ...
          <button
            ...
            title={t("pages.orgChart.zoomOut")}
            aria-label={t("pages.orgChart.zoomOut")}
          >
          ...
          <button
            ...
            onClick={fitToScreen}
            title={t("pages.orgChart.fitToScreen")}
            aria-label={t("pages.orgChart.fitChartToScreen")}
          >
